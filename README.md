...
OpenClaw 是一款专为 [RayNeo X3 Pro](https://www.rayneo.com/) AR 眼镜打造的智能语音 AI 助手。
...
| 密钥分级 | `openclaw.conf`（运行时）> `AppSettings`（用户设置）> `BuildConfig`（编译时）> 代码默认值 |
...
OpenClaw 依赖 OpenClaw 智能体网关提供 LLM 流式对话能力。
...
或直接在眼镜 Launcher 中找到 OpenClaw 应用打开。单击镜腿开始说话，AI 回复将实时渲染在眼前。

### 更新配置

如需更换 API Key 或切换服务器，修改本地的 `openclaw.conf`，重新执行第五、六步即可，无需重新安装。
...
├── openclaw.conf.template                   运行时配置模板
...
[MIT License](LICENSE) © 2026 OpenClaw Contributors
